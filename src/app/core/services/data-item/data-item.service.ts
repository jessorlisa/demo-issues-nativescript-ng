...
const GENERATED_ITEMS_COUNT = 150;
const GROUPS_COUNT = 3;
const SPECIAL_ITEM_INTERVAL = 4;
const generateItems = (itemPrefix?: string): DataItem[] => {
    const generatedItems = [];

    for (let i = 1; i <= GENERATED_ITEMS_COUNT; i++) {
        generatedItems.push(new DataItem(
            i,
            (itemPrefix ? itemPrefix : '') + (i % SPECIAL_ITEM_INTERVAL === 0 ? `Special Item ${i}` : `Item ${i}`),
...
export class DataItemService {
    ...
    getGeneratedDataItems(itemPrefix?: string): DataItem[] {
        return generateItems(itemPrefix);
    }
}
